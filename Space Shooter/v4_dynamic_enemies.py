...
pg.init()
pg.mixer.init()  # initialize the mixer for sounds
screen_x, screen_y = 800, 300
...
laser_sound = pg.mixer.Sound("laser.mp3")
...
no_of_enemies = 5
enemy_x_multiplier = screen_x // (no_of_enemies+1)
...
enemies_initial_x = [i*enemy_x_multiplier for i in range(1, no_of_enemies+1)]
enemy_dir = [1 if i %2 == 0 else -1  for i in range(no_of_enemies)]  # 1 for right, -1 for left
...
def create_enemies():
    ...
    for i in range(no_of_enemies):
        enemy = pg.Rect(enemies_initial_x[i], enemy_y, enemy_height, enemy_width)
...
def player_bullet_fire():
    if len(player_bullets) >= 3:
        return
    bullet = pg.Rect(player_x + (player_width//2), player_y - 10, 4, 10)
    player_bullets.append(bullet)
    laser_sound.play(fade_ms=500)  # play the laser sound effect
...
def is_enemy_collided_with_another(enemy):
    for other in enemies:
        if other != enemy and other.colliderect(enemy):
            return True
    return False
...
def move_enemy():
    ...
    for i in range(len(enemies)):
        if enemy_dir[i] == 1:
            enemies[i].x += 2
            if enemies[i].x >= enemies_initial_x[i]+(enemy_x_multiplier//2):
                enemy_dir[i] = -1
            if is_enemy_collided_with_another(enemies[i]):
                enemy_dir[i] = -1
        else:
            enemies[i].x -= 2
            if enemies[i].x <= enemies_initial_x[i]-(enemy_x_multiplier//2):
