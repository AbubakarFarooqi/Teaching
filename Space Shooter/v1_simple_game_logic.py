...
def create_enemies():
    enemy = pg.Rect(e1_x, e1_y, 20, 20)
    enemies.append(enemy)
    enemy = pg.Rect(e2_x, e2_y, 20, 20)
    enemies.append(enemy)
    enemy = pg.Rect(e3_x, e3_y, 20, 20)
    enemies.append(enemy)

def display_enemies():
    for enemy in enemies:
        pg.draw.rect(window, RED, enemy)
...
def player_rect():
    return pg.Rect(player_x, player_y, 20, 20)
...
def control_player():
    ...
    if keys[pg.K_LEFT] and player_x > 0:
        player_x -= 5
    if keys[pg.K_RIGHT] and player_x < screen_x - 20:
        player_x += 5
